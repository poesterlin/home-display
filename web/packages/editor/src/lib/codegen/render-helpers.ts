...
export function generateRenderHelpers(project: Project): string {
  ...
  lines.push(`  it.fill_rectangle(x, y, w, h, Theme::BACKGROUND_SECONDARY);`);
  lines.push(``);
  ...
  lines.push(`  if (label) {`);
  lines.push(`    it.printf(x + 10, y + 15, id(font_small), Theme::ACCENT, TextAlign::TOP_LEFT, "%s", label);`);
  lines.push(`  }`);
  lines.push(`}`);
  lines.push(``);
  ...
  lines.push(`  auto time = id(esptime).now();`);
  lines.push(`  it.strftime(10, 20, id(font_medium), Theme::FOREGROUND, TextAlign::CENTER_LEFT, "%H:%M", time);`);
  lines.push(`  it.strftime(230, 20, id(font_small), Theme::FOREGROUND_MUTED, TextAlign::CENTER_RIGHT, "%d.%m.%y", time);`);
  lines.push(`}`);
  lines.push(``);
  ...
  lines.push(`  it.fill_rectangle(0, 0, 240, 40, Theme::BACKGROUND);`);
  lines.push(`  it.line(0, 40, 240, 40, Theme::ACCENT);`);
  lines.push(`  `);
  lines.push(`  // Back button`);
  lines.push(`  it.rectangle(5, 5, 60, 30, Theme::ACCENT);`);
  lines.push(`  it.print(35, 20, id(font_small), Theme::ACCENT, TextAlign::CENTER, "< BACK");`);
  lines.push(`  `);
  lines.push(`  // Title`);
  lines.push(`  it.printf(150, 20, id(font_medium), Theme::FOREGROUND, TextAlign::CENTER, "%s", title);`);
  lines.push(`}`);
