<script lang="ts">
  ...
  const imageUrl = $derived(() => {
    ...
    if (component.file.startsWith("mdi:") || component.file.startsWith("mdil:")) {
      // Basic MDI icon display - requires a font-awesome or similar setup
      // For now, just show text representation
      return component.file;
    } else if (component.file.startsWith("http://") || component.file.startsWith("https://")) {
...
<Draggable {component}>
  <div
    ...
    style:background-color={component.image_type === 'BINARY' ? 'darkgrey' : 'lightgrey'}
  ...
  >
    {#if imageUrl.startsWith('mdi:') || imageUrl.startsWith('mdil:')}
      <span style="font-size: 24px; color: black;">{imageUrl}</span>
    {:else if imageUrl.startsWith('http://') || imageUrl.startsWith('https://')}
      <img src={imageUrl} alt="preview" style="max-width: 100%; max-height: 100%; object-fit: contain;" />
    {:else}
      <span style="color: black;">IMG: {component.file}</span>
    {/if}
  </div>
</Draggable>
